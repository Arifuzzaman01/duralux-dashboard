...
//  console.log(inventoryData);
  const pendingColumns = [
    { title: 'Product', dataKey: 'productName', key: 'productName', render: (text, record) => (
      <div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mt-8 px-3">
      ...
      <Card className="lg:col-span-7 shadow-sm border-none" title="Product Performance">
        <Tabs 
        ...
        <div className="h-[350px] mt-4">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart
              data={salesTab === 'best' ? inventoryData.bestSelling : inventoryData.lowSelling}
            ...
            >
              ...
              <Legend />
              <Area type="monotone" dataKey="amt" fill="#1cb811" stroke="#91d5ff" />
              <Bar dataKey="sales" barSize={30} fill="#10b981" radius={[4, 4, 0, 0]} />
              <Line type="monotone" dataKey="stock" stroke="#1cb811" strokeWidth={2} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      <Card className="lg:col-span-5 shadow-sm border-none" title="Stock & Order Status">
        <Tabs 
          defaultActiveKey="out" 
          items={[
            { 
              label: (<span>Out of Stock <Badge count={inventoryData?.outOfStock?.length} offset={[10, -5]} size="small" /></span>), 
              key: 'out',
              children: (
                <ul className="space-y-3 mt-2">
                  {inventoryData.outOfStock.map(item => (
                    <li key={item.id} className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
                      <div>
                        <p className="font-semibold text-gray-800 m-0">{item.name}</p>
        ...
        <div className="mt-8 pt-4 border-t border-gray-100 flex justify-between">
          ...
          <div className="text-center border-x px-8">
            ...
            <p className="text-xl font-bold text-red-500">{inventoryData.outOfStock.length}</p>
          </div>
          <div className="text-center">
